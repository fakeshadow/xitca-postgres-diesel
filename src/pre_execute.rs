...
use crate::{connection::Meta, error, serialize::ToSqlHelper};

const FAKE_OID: u32 = 0;

pub(crate) trait PreExecute: Send {
    fn pre_execute<Q>(
        &mut self,
        query: Q,
        meta: &Meta,
...
impl PreExecute for Transaction<'_, PoolConnection<'_>> {
    async fn prepare(&mut self, sql: &str, tys: &[Type]) -> QueryResult<CachedStatement> {
        xitca_postgres::Statement::named(sql, tys)
            .execute(self)
            .await
...
#[derive(Clone)]
pub(crate) struct BindValueIter<'a> {
