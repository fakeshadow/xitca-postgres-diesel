    ...
    Conn: AsyncConnection<TransactionManager = Self>,
{
    ...
    fn begin_transaction<'c, 'f>(conn: &'c mut Conn) -> BoxFuture<'f, QueryResult<()>>
    where
        'c: 'f,
    {
        Box::pin(async {
    ...
    fn rollback_transaction<'c, 'f>(conn: &'c mut Conn) -> BoxFuture<'f, QueryResult<()>>
    where
        'c: 'f,
    {
        Box::pin(async {
    ...
    fn commit_transaction<'c, 'f>(conn: &'c mut Conn) -> BoxFuture<'f, QueryResult<()>>
    where
        'c: 'f,
    {
        Box::pin(async {
